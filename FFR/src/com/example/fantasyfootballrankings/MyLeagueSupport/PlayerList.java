...
public class PlayerList {
	...
	public static void setPlayerInfoList(View res, Context c, ImportedTeam n) {
		...
		adapter = new SimpleAdapter(cont, data, R.layout.web_listview_item,
				new String[] { "main", "sub" }, new int[] { R.id.text1,
						R.id.text2 });
		final ListView list = (ListView) res
				.findViewById(R.id.imported_teams_players);
		list.setAdapter(adapter);
		final Spinner pos = (Spinner) res.findViewById(R.id.player_pos_spinner);
		final Spinner status = (Spinner) res
				.findViewById(R.id.player_status_spinner);
		final NDSpinner sortSp = (NDSpinner) res
				.findViewById(R.id.player_sort_spinner);
		pos.clearFocus();
		status.clearFocus();
		list.clearFocus();
		sortSp.clearFocus();
		...
		final OnItemSelectedListener l = new OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> arg0, View arg1,
				...
				populatePlayerList(list, pos, status.getSelectedItem()
						.toString(), sortSp.getSelectedItem().toString());
			}
		...
		final OnItemSelectedListener l2 = new OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> arg0, View arg1,
				...
				populatePlayerList(list, pos.getSelectedItem().toString(),
						status, sortSp.getSelectedItem().toString());
			}
		...
		final OnItemSelectedListener l3 = new OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> arg0, View arg1,
				...
				populatePlayerList(list, pos.getSelectedItem().toString(),
						status.getSelectedItem().toString(), sort);
				if (sort.equals("Custom")) {
					pos.setClickable(false);
					Toast.makeText(
							cont,
							"Note position now can only be set through the pop up, not through the drop down here until you choose"
									+ " a different sorting factor",
		...
		graph.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				SortHandler.buildGraph(cont, adapter,
						(sortSp.getSelectedItem()).toString(),
						(pos.getSelectedItem()).toString(),
						" with plotted Projection");
			}
		});
	}
	 ...
	 * @param list
	 */
	public static void setPlayerAdapter(List<Map<String, String>> data,
			ListView list) {
		adapter = new SimpleAdapter(cont, data, R.layout.web_listview_item,
				new String[] { "main", "sub" }, new int[] { R.id.text1,
						R.id.text2 });
		...
		list.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
					long arg3) {
				String name = ((TextView) ((RelativeLayout) arg1)
				...
				obj.outputResults(name + ", " + posTeam, true, (Activity) cont,
						ImportLeague.holder, false, false, false);
			}
		});
	}

	/**
	 * Handles the logic of populating the player list to fill the selected
	 * team/status...etc.
	 */
	public static void populatePlayerList(ListView list, String pos,
		...
		if (sortFactor.equals("Custom")) {
			...
			SortHandler.initialPopUp(cont, ImportLeague.holder,
					R.id.imported_teams_players, false, flag, newImport);
		} else {
			if (sortFactor.equals("PAA")) {
				players = new PriorityQueue<PlayerObject>(300,
						new Comparator<PlayerObject>() {
							...
							public int compare(PlayerObject a, PlayerObject b) {
								...
								if (a.values.points == 0
										&& b.values.points == 0) {
									return 0;
								}
								if (a.values.paa > b.values.paa
										|| (b.values.points == 0 && a.values.points > 0)) {
									return -1;
								}
								if (a.values.paa < b.values.paa
										|| (a.values.points == 0 && b.values.points > 0)) {
									return 1;
								}
								return 0;
							}
						...
						new Comparator<PlayerObject>() {
							...
							public int compare(PlayerObject a, PlayerObject b) {
								...
								if (a.values.rosRank > b.values.rosRank
										|| (b.values.rosRank == 0 && a.values.rosRank > 0)) {
									return 1;
								}
								if (a.values.rosRank < b.values.rosRank
										|| (a.values.rosRank == 0 && b.values.rosRank > 0)) {
									return -1;
								}
								return 0;
							}
			...
			while (!players.isEmpty()) {
				...
				datum.put("main", df.format(iter.values.points) + ":  "
						+ iter.info.name);
				...
				if (status.equals("Free Agents")
						&& !subInfo.toString().contains("Free Agent")) {
					continue;
				}
				if (status.equals("On Team")
						&& subInfo.toString().contains("Free Agent")) {
					...
					if (!iter.info.adp.contains("Not set")) {
						...
						if (!iter.info.adp.equals(Constants.BYE_WEEK)
								&& ((!ImportLeague.holder.isRegularSeason && ImportLeague.holder.sos
										.keySet().contains(
												iter.info.team + ","
														+ iter.info.position)) || (ImportLeague.holder.isRegularSeason && ImportLeague.holder.sos
										.keySet().contains(
												iter.info.adp + ","
														+ iter.info.position)))) {
							if (!ImportLeague.holder.isRegularSeason) {
								subInfo.append("SOS: "
										+ ImportLeague.holder.sos
												.get(iter.info.team + ","
														+ iter.info.position));
							}
							if (ImportLeague.holder.isRegularSeason) {
