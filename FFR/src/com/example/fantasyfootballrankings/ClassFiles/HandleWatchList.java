...
public class HandleWatchList {
	 ...
	 * @param watchList
	 */
	public static void handleWatchInit(final Storage holder,
			final Context cont, final List<String> watchList) {
		final Dialog dialog = new Dialog(cont);
		dialog.requestWindowFeature(Window.FEATURE_NO_TITLE);
		dialog.setContentView(R.layout.search_output);
		final ListView listWatch = (ListView) dialog
		...
		if (watchList.size() == 0
				|| (watchList.size() > 0 && !holder.parsedPlayers
						.contains(watchList.get(0)))) {
		...
		back.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if (watchList.size() > 0
					...
					Toast.makeText(cont, "Watch List Cleared",
							Toast.LENGTH_SHORT).show();
					watchList.clear();
					...
					Toast.makeText(cont, "No players in the list to clear",
							Toast.LENGTH_SHORT).show();
				}
			}
		});
		SimpleAdapter mAdapter = display(dialog, watchList, holder, listWatch,
				cont);
	 ...
	 */
	public static SimpleAdapter display(Dialog dialog, List<String> watchList,
			Storage holder, ListView listWatch, Context cont) {
		...
		while (!totalList.isEmpty()) {
			...
			if (selected) {
				if (Draft.isDrafted(iter.info.name, holder.draft)) {
					...
					datum.put("info", iter.info.position + " - "
							+ iter.info.team);
					datum.put("hidden", "");
				}
			} else {
		...
		final SimpleAdapter adapter = new SimpleAdapter(cont, dataSet,
				R.layout.web_listview_item, new String[] { "name", "info",
						"hidden" }, new int[] { R.id.text1, R.id.text2,
						R.id.text3 });
	...
	public static void handleListSelect(final Storage holder,
		...
		listview.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
				...
				String nameText = ((TextView) ((RelativeLayout) arg1)
				...
				obj.outputResults(selected + ", " + posTeam, true,
						(Activity) cont, holder, true, true, false);
			}
		});
				...
				new SwipeDismissListViewTouchListener.OnDismissCallback() {
					...
					public void onDismiss(ListView listView,
						...
						if (watchList.size() == 0) {
							dialog.dismiss();
							Toast.makeText(cont,
									"No players left in the watch list",
									Toast.LENGTH_SHORT).show();
						}
						Toast.makeText(cont, "Removing " + name,
								Toast.LENGTH_SHORT).show();
						for (Map<String, String> datum : Rankings.data) {
