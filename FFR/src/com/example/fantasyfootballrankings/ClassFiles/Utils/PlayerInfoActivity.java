...
/**
 * A little library to handle the rendering of the player view/like stats in a
 * pop up
 * 
 * @author Jeff
 * 
...
public class PlayerInfoActivity {
	...
	/**
	 * Does the real work, fetching the data asynchronously, sorting it
	 * appropriately, and displays it in a simple way on the pop up
	 * 
	 * @param cont
	 */
	public static void displayStats(Context cont) {
		...
		EntityUtils.getEntities((Activity) cont, 0, 200,
				SortOrder.TOTAL_ACTIVITY, new EntityListListener() {
					...
					public void onList(ListResult<Entity> result) {
						...
						if (sorted.size() > 0) {
							...
							while (!sorted.isEmpty() && i < 15) {
								...
								i++;
								data.append(i + ". " + elem.getDisplayName()
										+ " - "
										+ elem.getEntityStats().getViews()
										+ " views\n");
							}
						}
						...
						while (!sortedComments.isEmpty() && i < 15) {
							...
							data.append(i + ". " + elem.getDisplayName()
									+ " - "
									+ elem.getEntityStats().getComments()
									+ " likes\n");
						}
	...
	public static PriorityQueue<Entity> sortEntitiesViews(List<Entity> input) {
		PriorityQueue<Entity> sorted = new PriorityQueue<Entity>(100,
				new Comparator<Entity>() {
					@Override
					public int compare(Entity a, Entity b) {
						if (a.getEntityStats().getViews() > b.getEntityStats()
								.getViews()) {
							return -1;
						}
						if (a.getEntityStats().getViews() < b.getEntityStats()
	...
	public static PriorityQueue<Entity> sortEntitiesLikes(List<Entity> input) {
		PriorityQueue<Entity> sorted = new PriorityQueue<Entity>(100,
				new Comparator<Entity>() {
					@Override
					public int compare(Entity a, Entity b) {
						if (a.getEntityStats().getLikes() > b.getEntityStats()
								.getLikes()) {
							return -1;
						}
						if (a.getEntityStats().getLikes() < b.getEntityStats()
	...
	public static PriorityQueue<Entity> sortEntitiesComments(List<Entity> input) {
		PriorityQueue<Entity> sorted = new PriorityQueue<Entity>(100,
				new Comparator<Entity>() {
					@Override
					public int compare(Entity a, Entity b) {
						if (a.getEntityStats().getComments() > b
								.getEntityStats().getComments()) {
							return -1;
						}
						if (a.getEntityStats().getComments() < b
								.getEntityStats().getComments()) {
	...
	public static PriorityQueue<Entity> sortEntitiesDraft(List<Entity> input) {
		PriorityQueue<Entity> sorted = new PriorityQueue<Entity>(100,
				new Comparator<Entity>() {
					@Override
					public int compare(Entity a, Entity b) {
						if (Integer.parseInt(a.getMetaData()) > Integer
								.parseInt(b.getMetaData())) {
							return -1;
						}
						if (Integer.parseInt(a.getMetaData()) < Integer
								.parseInt(b.getMetaData())) {
		...
		for (Entity elem : input) {
			if (elem.getMetaData() != null && elem.getMetaData().length() > 0
					&& ManageInput.isInteger(elem.getMetaData())
					&& Integer.parseInt(elem.getMetaData()) > 0) {
				sorted.add(elem);
			}
		}
		return sorted;
