...
/**
 * A class that will hold the structures to store a draft class, in addition to
 * value and salary remaining
 * 
 * @author Jeff
 * 
 */
public class Draft {
	...
	public static int playersDrafted(Draft draft) {
		return draft.qb.size() + draft.rb.size() + draft.wr.size()
				+ draft.te.size() + draft.def.size() + draft.k.size();
	}
	...
	/**
	 * Adjusts the values of a draft class given values of a pick that's made
	 * 
	 * @param valuePlayer
	 * @param paid
	 */
	...
	public static void resetDraft(Draft draft, Storage holder, Context cont) {
		Toast.makeText(cont, "Draft information reset", Toast.LENGTH_SHORT)
				.show();
		draft.qb.clear();
		draft.rb.clear();
		draft.wr.clear();
	 ...
	 */
	public static void undraft(final Dialog dialog, final Storage holder,
			final Context cont) {
		...
		for (String name : holder.draft.ignore) {
			...
			if (isAuction) {
				datum.put("main", df.format(p.values.secWorth) + ":  "
						+ p.info.name);
			} else {
		...
		final SimpleAdapter adapter = new SimpleAdapter(cont, data,
				R.layout.web_listview_item, new String[] { "main", "sub" },
				new int[] { R.id.text1, R.id.text2 });
		listWatch.setAdapter(adapter);
		// ArrayAdapter<String> mAdapter = display(dialog, holder.draft.ignore,
		// holder, listWatch, cont);
	 ...
	 * @param data
	 */
	public static void handleListSelect(final Storage holder,
			final Context cont, ListView listview, final Dialog dialog,
			final SimpleAdapter adapter, final List<Map<String, String>> data) {
				...
				new SwipeDismissListViewTouchListener.OnDismissCallback() {
					...
					public void onDismiss(ListView listView,
						...
						for (int position : reverseSortedPositions) {
							...
							dialog.dismiss();
							undraftPlayer(name, new Dialog(cont,
									R.style.RoundCornersFull), holder,
									(Activity) cont, adapter, position, data);
	 ...
	 * @param data
	 */
	public static void undraftPlayer(final String name, final Dialog dialog,
			final Storage holder, final Activity cont,
			final SimpleAdapter adapter, final int position,
		...
		close.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				for (PlayerObject player : holder.players) {
					if (player.info.name.equals(name)) {
						if (isAuction) {
							datum.put("main", df.format(player.values.secWorth)
									+ ":  " + player.info.name);
						} else {
							datum.put("main", df.format(player.values.ecr)
									+ ":  " + player.info.name);
						}
		...
		someone.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				Toast.makeText(cont, "Undrafting " + name, Toast.LENGTH_SHORT)
						.show();
			...
			public void onClick(View v) {
				if (isAuction) {
					draftedByMe(name, holder, cont, new Dialog(cont,
							R.style.RoundCornersFull), adapter, position,
							false, data);
				} else {
	 ...
	 * @param data
	 */
	public static void draftedByMe(final String name, final Storage holder,
			final Activity cont, final Dialog popup,
			final SimpleAdapter adapter, final int position,
			final boolean flag, final List<Map<String, String>> data) {
		...
		back.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				popup.dismiss();
				undraftPlayer(name, new Dialog(cont), holder, cont, adapter,
						position, data);
			}
		});
	 ...
	 */
	public static void handleUnDraftingMe(int val, Storage holder, String name,
