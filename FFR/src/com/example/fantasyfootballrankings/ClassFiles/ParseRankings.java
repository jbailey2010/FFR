...
/**
 * This is actually where the rankings are parsed One function per web site to
 * be parsed
 * 
 * @author Jeff
 * 
...
public class ParseRankings {
	...
	/**
	 * This is where the magic happens. This sets the names/fetches them if
	 * they're stored (not based on user input as it won't change), then fetches
	 * the players, and runs high-level stuff on it.
	 * 
	 * @param cont
	 *            the context used to work with the saving
	 * @throws IOException
	 * @throws XPatherException
	 */
	public static void runRankings(final Storage holder, final Context cont)
	 ...
	 * it exists in the pq or not, it calls handlePlayer to do what's
	 * appropriate. Abstracted as it was called in all parsers.
	 * 
	 * @param holder
	 *            the storage written to
	 * @param playerName
	...
	/**
	 * A generally abstracted function that has two cases -- if it exists in the
	 * priority queue already and if it doesn't. If so, it removes it from the
	 * pq, standardizes stuff, adjusts the values, and re-adds it, if it isn't
	 * there, it sets extremes and adds it.
	 * 
	 * @param holder
	 *            the storage to be added to
	 * @param newPlayer
	 *            the newly created player to be worked with
	 * @param match
	 *            the already existent match to use if need be
	 */
	public static void handlePlayer(Storage holder, PlayerObject newPlayer,
			PlayerObject match) {
		if (match != null) {
			int flag = BasicInfo.standardAll(newPlayer.info.team,
					newPlayer.info.position, match.info);
			if (flag == -1) {
				for (PlayerObject iter : holder.players) {
					if (iter.info.name.equals(newPlayer.info.name)
							&& !iter.info.position.equals(match.info.position)) {
	...
	/**
	 * This adjusts teams to a standard value. MAY NEED ADJUSTING AS NEW PARSERS
	 * ADDED
	 * 
	 * @param team
	 *            the team to check
