...
public class Simulator {
	...
	public static void simulatorInit(final Context cont, final Storage holder){
	    ...
	    final AutoCompleteTextView player = (AutoCompleteTextView)dialog.findViewById(R.id.simulator_input);
		...
		submit.setOnClickListener(new OnClickListener(){
			...
			public void onClick(View v) {
				...
				if(!ManageInput.isInteger(rdStr) || !ManageInput.isInteger(pickStr)){
					Toast.makeText(cont, "Please enter numbers for round and pick", Toast.LENGTH_SHORT).show();
					return;
				}
				int rd = Integer.parseInt(rdStr);
				int sel = Integer.parseInt(pickStr);
				if(sel > ReadFromFile.readRoster(cont).teams){
					Toast.makeText(cont, "The selection can't be later than the number of teams", Toast.LENGTH_SHORT).show();
					return;
				}
				if(rd * sel > 200){
					Toast.makeText(cont, "Pick too high, please enter a selection of at highest 200", Toast.LENGTH_LONG).show();
					return;
				}
				String name = player.getText().toString();
				if(!holder.parsedPlayers.contains(name)){
					Toast.makeText(cont, "Please enter a valid player name", Toast.LENGTH_SHORT).show();
					return;
				}
				ParsingAsyncTask stupid = new ParsingAsyncTask();
				ParseADP task = stupid.new ParseADP((Activity) cont, holder, header);
