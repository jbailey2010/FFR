...
public class StorageAsyncTask {
	...
	public class WriteNewPAA extends AsyncTask<Object, Void, Void> {
		...
		protected void onPostExecute(Void result) {
			...
			if (!Home.holder.isRegularSeason && !isPI) {
				Toast.makeText(
						cont,
						"Note, this changed some auction values. To reflect this, refresh the rankings",
	...
	/**
	 * This handles the reading of the draft data in the background
	 * 
	 * @author Jeff
	 * 
	 */
	public class ReadDraft extends AsyncTask<Object, Void, Storage> {
		...
		protected void onPostExecute(Storage result) {
			if (teamFail) {
				Toast.makeText(
						act,
						"An error occurred in saving. You may need to refresh ranks. Make sure you have a strong internet connection",
						Toast.LENGTH_LONG).show();
			}
			if (flag == 0) {
		...
		protected Storage doInBackground(Object... data) {
			...
			if (holder.maxProj() > 65) {
				...
				if (!perSet[0].equals("Doesn't matter")
						&& individual.length > 4) {
					// Qb fetching
					ReadFromFile.handleDraftReading(individual[0],
							holder.draft.qb, holder);
					// Rb fetching
					ReadFromFile.handleDraftReading(individual[1],
							holder.draft.rb, holder);
					// Wr fetching
					ReadFromFile.handleDraftReading(individual[2],
							holder.draft.wr, holder);
					// Te fetching
					ReadFromFile.handleDraftReading(individual[3],
							holder.draft.te, holder);
					// Def fetching
					ReadFromFile.handleDraftReading(individual[4],
							holder.draft.def, holder);
					// K fetching
					ReadFromFile.handleDraftReading(individual[5],
							holder.draft.k, holder);
					// Ignore fetching
					holder.draft.ignore.clear();
					for (String name : individual[6]) {
						if (name.length() > 3
								&& !holder.draft.ignore.contains(name)) {
							holder.draft.ignore.add(name);
						}
					}
					// Values
					holder.draft.remainingSalary = Integer
							.parseInt(individual[7][0]);
					holder.draft.value = Double.parseDouble(individual[8][0]);
				}
			}
			System.out
					.println(System.nanoTime() - start + " to read from file");
			return holder;
		}
	}
}