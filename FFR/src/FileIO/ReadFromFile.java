...
/**
 * A library of all the functions that will read from file
 * 
 * @author Jeff
 * 
 */
public class ReadFromFile {
	...
	public static void readDraft(Storage holder, Context cont) {
		...
		if (!perSet[0].equals("Doesn't matter") && individual.length > 4) {
			// Qb fetching
			ReadFromFile.handleDraftReading(individual[0], holder.draft.qb,
					holder);
			// Rb fetching
			ReadFromFile.handleDraftReading(individual[1], holder.draft.rb,
					holder);
			// Wr fetching
			ReadFromFile.handleDraftReading(individual[2], holder.draft.wr,
					holder);
			// Te fetching
			ReadFromFile.handleDraftReading(individual[3], holder.draft.te,
					holder);
			// Def fetching
			ReadFromFile.handleDraftReading(individual[4], holder.draft.def,
					holder);
			// K fetching
			ReadFromFile.handleDraftReading(individual[5], holder.draft.k,
					holder);
			// Ignore fetching
	 ...
	 * If not, it calls a function to fetch them and write to file. It's set up
	 * this way to minimize other calls such that only one function needs to be
	 * called, yet running time is minimized this way.
	 * 
	 * @param holder
	 *            holds the array to be written to
	 * @param cont
	 *            holds the context to be used to write to file.
	 * @throws IOException
	 */
	 ...
	 * re-adds them to the priority queue, but if it isn't, it calls the
	 * runRankings function, which is the function that calls all the rankings
	 * functions and the high level stuff
	 */
	public static void fetchPlayers(Set<String> checkExists, Storage holder,
			Context cont, int i) {
	 ...
	 * @param holder
	 */
	public static void handleDraftReading(String[] individual,
			List<PlayerObject> target, Storage holder) {
		target.clear();
	 ...
	 * 
	 * @param cont
	 *            the context from which it reads
	 * @return the size
	 */
	...
	public static void readTeamData(Storage holder, Context cont) {
		...
		try {
			...
			for (String team : perLevel) {
				holder.draftClasses.put(team.split("##")[0],
						team.split("##")[1]);
			}
			// SOS
			...
			for (String team : perLevel) {
				if (team.split("##")[0].equals(" ")) {
					...
					holder.sos.put(team.split("##")[0],
							Integer.parseInt(team.split("##")[1]));
				}
			}
			// Bye
